...
## 新フォーマットについて
東京都は、2021-11-25 の分から、  
https://www.fukushihoken.metro.tokyo.lg.jp/iryo/kansen/corona_portal/info/kunishihyou.html  
で公開しているデータのフォーマットを変更しました。

しばらく csv 化は様子見とします。  
私の pdf→csv の処理はあまり柔軟性が高くないので、新フォーマットに対応できるかどうかは不明です。  
このまま第6波が来ないのなら、こうしたデータの重要性もさほど高くないとも言えます。

なお、東京都は毎日、前日公開した pdf データを削除してしまうのですが、この pdf データのダウンロードはしておく予定です。

## 旧フォーマットについて
2021-11-24 の分までのデータについてです。
...
- (c) 2021-07-06 以降、2021-11-24 までについては、全ての日のデータがあります。
...
2021-11-26
東京都は、2021-11-25 日分から、  
https://www.fukushihoken.metro.tokyo.lg.jp/iryo/kansen/corona_portal/info/kunishihyou.html  
で公開しているデータのフォーマットを変更しました。
これに対しては、冒頭の「新フォーマットについて」をご参照下さい。
...
2021-08-02～2021-11-24 については、公開された pdf から csv 化しました。
...
具体的には、pdf データ（[例えばこちら](https://web.archive.org/web/20210829113548/https://www.fukushihoken.metro.tokyo.lg.jp/iryo/kansen/corona_portal/info/kunishihyou.files/kuni0828.pdf)）の下から 3 行、「病床全体の使用率」「入院率」「重症者用病床の使用率」を csv 化した箇所の一部では、表頭として「・・・_分子」「・・・_分母」という機械的な名称を採用しています。
